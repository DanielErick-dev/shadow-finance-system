...
export default function AddRecurringExpenseForm({ onSuccess, onCancel, initialData }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (initialData) {
        await editRecurring.mutateAsync({ id: initialData.id, data: dataToSend })
      } else {
        await addRecurring.mutateAsync(dataToSend)
      }
      onSuccess()
    } catch {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <button type="button" onClick={onCancel}
          className="text-slate-400 hover:text-purple-400 transition-all duration-200 hover:scale-110 rounded-full p-1"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <Label htmlFor="amount" className="block text-sm font-medium text-slate-300 mb-2">Valor</Label>
          <Input type="number" name="amount" value={formData.amount} onChange={handleChange} min={0} required step="0.01" placeholder="0.00"
            className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
          />
        </div>
        <div>
          <Label htmlFor="category_id" className="block text-sm font-medium text-slate-300 mb-2">Categoria</Label>
        ...
        <div>
          ...
          <select id="due_day" name="due_day" value={formData.due_day} onChange={handleChange} required
            className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
          >
        ...
        <div>
          <Label htmlFor="start_date" className="block text-sm font-medium text-slate-300 mb-2">Data de Início</Label>
          <Input type="date" id="start_date" name="start_date" value={formData.start_date} onChange={handleChange} required
            className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
          />
        </div>
        <div>
          <Label htmlFor="end_date" className="block text-sm font-medium text-slate-300 mb-2">Data de Término (opcional)</Label>
      ...
      <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-lg border border-slate-700">
        <Input type="checkbox" id="active" name="active" checked={formData.active} onChange={handleChange}
          className="w-5 h-5 bg-slate-700 border-2 border-slate-600 rounded text-purple-500 focus:ring-2 focus:ring-purple-500"
        />
      ...
      <div className="flex items-center justify-end gap-3 pt-4">
        <button type="button" onClick={onCancel} disabled={isSubmitting}
          className="px-6 py-2 bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white border border-slate-600 rounded-lg font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Cancelar
        </button>
        <button type="submit" disabled={isSubmitting}
          className="px-6 py-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white border border-purple-500 rounded-lg font-medium transition-all duration-200 hover:shadow-lg hover:shadow-purple-500/30 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
