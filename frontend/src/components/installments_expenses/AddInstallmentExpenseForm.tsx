...
export default function AddInstallmentExpenseForm({ onSaveSuccess, onCancel }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Number(formData.installments_quantity) <= 1) return toast.error("A quantidade de parcelas deve ser maior que 1.")

    setIsSubmitting(true)
    try {
      await addInstallment.mutateAsync(formData)
      onSaveSuccess()
    } catch {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <Input name="name" id="name" onChange={handleInputChange} value={formData.name}
                placeholder="Ex: Notebook Gamer" disabled={isSubmitting}
            ...
            <div>
              <Label htmlFor="total_amount" className="text-slate-300 font-medium mb-2 block">Valor Total (R$)</Label>
              <Input id="total_amount" name="total_amount" value={formData.total_amount} onChange={handleInputChange}
                type="number" placeholder="0,00" disabled={isSubmitting}
                className="bg-slate-800/60 border-slate-600/50 text-slate-100 placeholder:text-slate-400 focus:border-purple-400 focus:ring-purple-400/20 rounded-xl"
              />
            </div>
            <div>
              <Label htmlFor="installments_quantity" className="text-slate-300 font-medium mb-2 block">Nº de Parcelas</Label>
              <Input id="installments_quantity" name="installments_quantity" value={formData.installments_quantity}
                onChange={handleInputChange} type="number" placeholder="12" disabled={isSubmitting}
                className="bg-slate-800/60 border-slate-600/50 text-slate-100 placeholder:text-slate-400 focus:border-purple-400 focus:ring-purple-400/20 rounded-xl"
              />
            </div>
            <div>
              <Label htmlFor="first_due_date" className="text-slate-300 font-medium mb-2 block">Vencimento da 1ª Parcela</Label>
              <Input id="first_due_date" name="first_due_date" value={formData.first_due_date}
                onChange={handleInputChange} type="date" disabled={isSubmitting}
                className="bg-slate-800/60 border-slate-600/50 text-slate-100 focus:border-purple-400 focus:ring-purple-400/20 rounded-xl"
              />
            </div>
            <div>
              <Label htmlFor="category_id" className="text-slate-300 font-medium mb-2 block">Categoria</Label>
              ...
              >
                <option value="" className="bg-slate-800">Nenhuma</option>
                {categories.map((cat) => (
