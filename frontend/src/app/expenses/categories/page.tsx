...
export default function CategoriesExpensesPage() {
    ...
    const handleDeleteClick = async (category: Category) => {
        const isConfirmed = await confirm({
            ...
            description: `Você tem certeza que deseja excluir a categoria: ${category.name}?`,
            ...
            <div className="min-h-screen text-slate-200 py-6">
                <div className="container mx-auto px-4 md:px-6 max-w-4xl">
                    <header className="flex flex-col sm:flex-row justify-between items-center mb-10 pb-5 border-b border-purple-800/50">
                        <h1 className="text-4xl font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400 mb-4 sm:mb-0 text-center w-full sm:w-auto">
                    ...
                    <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-lg shadow-purple-900/20 overflow-hidden">
                        <table className="w-full text-left">
                            ...
                            <tbody>
                                {categories.map((category) => (
                                    <tr key={category.id} className="border-b border-slate-800 last:border-b-0 hover:bg-slate-800/70 transition-colors">
                                        <td className="p-4 text-slate-400">{category.name}</td>
                                        <td className="p-4">
                                            <div className="flex items-center gap-3">
                                                <button className="p-2 rounded-full hover:bg-slate-700 group" onClick={() => setCategoryToEdit(category)}>
                                                    <Pencil className="w-4 h-4 text-slate-400 transition-colors group-hover:text-blue-400 cursor-pointer" />
                                                </button>
                                                <button className="p-2 rounded-full hover:bg-slate-700 group" onClick={() => handleDeleteClick(category)}>
                                                    <Trash2 className="w-4 h-4 text-slate-400 transition-colors group-hover:text-red-300 cursor-pointer" />
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        {categories.length === 0 && (
                            <div className="text-center text-slate-500 p-8 flex flex-col items-center gap-4">
                                ...
                                <p>Nenhuma categoria cadastrada. Clique no botão acima para adicionar.</p>
                            </div>
                        )}
                    </div>
                </div>
