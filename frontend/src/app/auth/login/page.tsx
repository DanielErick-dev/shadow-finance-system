...
export default function Login() {
    ...
    const handleDemonstrativeUser = async (event: React.FormEvent) => {
        event.preventDefault();
        setError(null)
        setIsSubmitting(true)
        try{
            await login('teste', 'test@123');
        } catch(error){
            setError('acesso negado! Credencias inválidas.');
        ...
        <div className="min-h-screen bg-black flex items-center justify-center p-4 font-mono">
            ...
            <div className="relative bg-gray-900/80 backdrop-blur-sm border border-purple-500/30 rounded-xl shadow-lg shadow-purple-500/10 w-full max-w-md overflow-hidden z-10">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <button
                        type="button"
                        onClick={handleDemonstrativeUser}
                        className="w-full bg-gradient-to-r from-gray-700 to-gray-600 hover:from-gray-600 hover:to-gray-500 text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:shadow-gray-500/20 transition-all duration-300 flex items-center justify-center mt-3"
                        disabled={isSubmitting}
                    >
                        <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
